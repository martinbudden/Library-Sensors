...
void test_map_axes_inversion()
{
    ...
    for (uint8_t axis_order = IMU_Base::XPOS_YPOS_ZPOS; axis_order <= IMU_Base::YNEG_XPOS_ZPOS_45; ++axis_order) {
        const xyz_t intermediate = IMU_Base::map_axes(input, axis_order);
        const xyz_t output =       IMU_Base::map_axes(intermediate, IMU_Base::axis_order_inverse(axis_order));
        TEST_ASSERT_EQUAL_FLOAT(input.x, output.x);
        TEST_ASSERT_EQUAL_FLOAT(input.y, output.y);
        TEST_ASSERT_EQUAL_FLOAT(input.z, output.z);
    }

    auto axis_order = IMU_Base::XPOS_YPOS_ZPOS_45;
    xyz_t intermediate = IMU_Base::map_axes(input, axis_order);
    xyz_t output =       IMU_Base::map_axes(intermediate, IMU_Base::axis_order_inverse(axis_order));
    TEST_ASSERT_EQUAL_FLOAT(input.x, output.x);
    TEST_ASSERT_EQUAL_FLOAT(input.y, output.y);
    TEST_ASSERT_EQUAL_FLOAT(input.z, output.z);

    axis_order = IMU_Base::YPOS_XNEG_ZPOS_45;
    intermediate = IMU_Base::map_axes(input, axis_order);
    output =       IMU_Base::map_axes(intermediate, IMU_Base::axis_order_inverse(axis_order));
    TEST_ASSERT_EQUAL_FLOAT(input.x, output.x);
    TEST_ASSERT_EQUAL_FLOAT(input.y, output.y);
    TEST_ASSERT_EQUAL_FLOAT(input.z, output.z);

    axis_order = IMU_Base::XNEG_YNEG_ZPOS_45;
    intermediate = IMU_Base::map_axes(input, axis_order);
    output =       IMU_Base::map_axes(intermediate, IMU_Base::axis_order_inverse(axis_order));
    TEST_ASSERT_EQUAL_FLOAT(input.x, output.x);
    TEST_ASSERT_EQUAL_FLOAT(input.y, output.y);
    ...
    axis_order = IMU_Base::YNEG_XPOS_ZPOS_45;
    intermediate = IMU_Base::map_axes(input, axis_order);
    output =       IMU_Base::map_axes(intermediate, IMU_Base::axis_order_inverse(axis_order));
    TEST_ASSERT_EQUAL_FLOAT(input.x, output.x);
    TEST_ASSERT_EQUAL_FLOAT(input.y, output.y);
...
void test_raw()
{
    ...
    IMU_Base::xyz_int32_t gyroRaw = imu.read_gyro_raw();
    ...
    IMU_Base::xyz_int32_t accRaw = imu.read_acc_raw();
