...
void test_lsm6ds3tr_c()
{
    ...
    const IMU_LSM6DS3TR_C imu(IMU_Base::XPOS_YPOS_ZPOS, spiFrequency, BusSpi::BUS_INDEX_0,
        BusSpi::spi_pins_t{});
#else
    constexpr uint8_t SDA_pin = 0;
    constexpr uint8_t SCL_pin = 0;
    const IMU_LSM6DS3TR_C imu(IMU_Base::XPOS_YPOS_ZPOS, BusI2c::i2c_pins_t{.sda=IMU_I2C_SDA_PIN, .scl=IMU_I2C_SCL_PIN});
#endif
