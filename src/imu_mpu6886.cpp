...
#if defined(LIBRARY_SENSORS_IMU_USE_SPI_BUS) || defined(LIBRARY_SENSORS_ImuMpu6886_USE_SPI_BUS)
ImuMpu6886::ImuMpu6886(uint8_t axis_order, uint32_t frequency, BusBase::bus_index_e spi_index, const BusSpi::stm32_spi_pins_t& pins) :
    ImuBase(axis_order, _bus),
    _bus(frequency, spi_index, pins)
{
}
ImuMpu6886::ImuMpu6886(uint8_t axis_order, uint32_t frequency, BusBase::bus_index_e spi_index, const BusSpi::spi_pins_t& pins) :
    ImuBase(axis_order, _bus),
    _bus(frequency, spi_index, pins)
{
}
#else
ImuMpu6886::ImuMpu6886(uint8_t axis_order, BusBase::bus_index_e i2c_index, const BusI2c::i2c_pins_t& pins, uint8_t I2C_address) :
    ImuBase(axis_order, _bus),
...
int ImuMpu6886::init(uint32_t target_output_data_rate_hz, uint8_t gyro_sensitivity, uint8_t acc_sensitivity, void* bus_mutex)
{
...
ImuBase::xyz_int32_t ImuMpu6886::read_acc_raw()
{
...
xyz_t ImuMpu6886::read_gyro_rps()
{
    mems_sensor_data_t gyro; // NOLINT(cppcoreguidelines-pro-type-member-init,hicpp-member-init,misc-const-correctness)
...
xyz_t ImuMpu6886::read_gyro_dps()
{
    return read_gyro_rps() * RADIANS_TO_DEGREES;
}

xyz_t ImuMpu6886::read_acc()
{
    mems_sensor_data_t acc; // NOLINT(cppcoreguidelines-pro-type-member-init,hicpp-member-init,misc-const-correctness)
...
FAST_CODE acc_gyro_rps_t ImuMpu6886::read_acc_gyro_rps()
{
    bus_semaphore_take(_bus_mutex);
...
*/
FAST_CODE acc_gyro_rps_t ImuMpu6886::get_acc_gyro_rps() const
{
    return acc_gyro_rpsFromRaw(_spiAccTemperatureGyroData.accGyro.value);
}

int32_t ImuMpu6886::readTemperatureRaw() const
{
...
float ImuMpu6886::readTemperature() const
{
...
xyz_t ImuMpu6886::gyroRPS_FromRaw(const mems_sensor_data_t::value_t& data) const
{
...
xyz_t ImuMpu6886::accFromRaw(const mems_sensor_data_t::value_t& data) const
{
#if defined(LIBRARY_SENSORS_IMU_FIXED_AXES_XPOS_YPOS_ZPOS)
    return xyz_t {
...
acc_gyro_rps_t ImuMpu6886::acc_gyro_rpsFromRaw(const acc_temperature_gyro_data_t::value_t& data) const
{
#if defined(LIBRARY_SENSORS_IMU_FIXED_AXES_XPOS_YPOS_ZPOS)
